...
const COUPON_OPTIONS = [0, 10, 15, 20, 25, 30, 35, 40, 45, 50]

const DiscountMenu = ({ index, coupon, onChange }: DiscountMenuProps) => (
  <select value={coupon} onChange={(e) => onChange(e.target.value, index)} className="w-20 h-10 border border-gray-200 rounded-[10px]">
    {COUPON_OPTIONS.map((val) => (
      <option key={val} value={String(val)}>{val === 0 ? 'ไม่ใช้' : `${val}%`}</option>
    ))}
  </select>
)
...
function App() {
  ...
  const handleChangeData = (name: string, value: string, pos: number) => {
    if(name === 'quantity') {
  ...
  const taxCalc = (price: number, coupon: string): PriceType => {
    ...
    if(privilageDiscount.includes("SVIP")){
      discount += taxPrice * 0.2
      vipDiscount += taxPrice * 0.2
    }
    if(privilageDiscount.includes("VIP")){
      discount += taxPrice * 0.1
      vipDiscount += taxPrice * 0.1
    }
    if(coupon !== '0') {
      const rate = parseInt(coupon) / 100
      couponDiscount = taxPrice * rate
      discount += couponDiscount
    }
    return {
      pricePerPlayer: Math.floor(basePrice + discount),
      allDiscountPrice: Math.floor(discount),
      vipDiscount: Math.floor(vipDiscount),
  ...
  const resultCalc = (): string => {
    const total = player
      ...
      .map(item => taxCalc(item.price * item.quantity, item.coupon).pricePerPlayer)
  ...
  const handleSelectedPlayerList = (pos: number) => {
    if(pos === -1) {
      ...
      if (!selectedAll) {
        setPlayer(player.map((item) => ({...item, selected: true})))
      }
      else {
        setPlayer(player.map((item) => ({...item, selected: false})))
      }
    }
    else {
      setPlayer(player.map((item, index) => {
        ...
        <table className="min-w-[1440px] xl:w-full table-auto">
          ...
          <tbody>
            {player.map((item, index) => {
              const calc = taxCalc(item.price * item.quantity, item.coupon)
              return (
                <tr className="bg-white border-b h-16" key={`player${index+1}`}>
                  ...
                  <td className="text-left px-4"><input name="price" type="number" className="focus:select-none border-gray-200 border rounded-lg p-2" value={item.price} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChangeData(e.target.name, e.target.value, index)} /></td>
                  ...
                  <td className="text-left px-4"><DiscountMenu index={index} coupon={item.coupon} onChange={handleDiscountSelected} /></td>
                  <td className="text-left px-4 font-bold text-red-600">-{CurrencyFormatter(tax_40_new(item.price * item.quantity))}</td>
                  <td className="text-left px-4 font-bold text-green-600">+{CurrencyFormatter(calc.vipDiscount)}</td>
                  <td className="text-left px-4 font-bold text-green-600">+{CurrencyFormatter(calc.couponDiscount)}</td>
                  <td className="text-left px-4 font-bold text-[#349beb]">{CurrencyFormatter(calc.pricePerPlayer)}</td>
                  <td className="px-4">
                    <button className="p-2 border border-gray-100 bg-white rounded-[10px]" onClick={() => removePlayer(index)}>
                    <svg width="14" height="16" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <g clipPath={`url(#clip_trash_${index})`}>
